"""
Decoding visualization orchestration functions.

High-level entry points for creating decoding visualizations from analysis results.
Coordinates calls to specialized plotting modules for comprehensive visualization workflows.
"""
...
def visualize_regression_results(
    ...
    """
    Create comprehensive visualizations for regression decoding results.
    
    Generates all standard regression decoding plots:
    - Prediction scatter plot
    - Per-subject performance
    - Residual diagnostics
    - Null distribution histogram (if null_r provided)
    - Calibration curve (if cal_metrics provided)
    - Bootstrap distributions (if bootstrap_results provided)
    
    Args:
        pred_df: DataFrame with 'y_true' and 'y_pred' columns
        per_subj_df: DataFrame with per-subject performance metrics
        pooled_metrics: Dictionary with pooled performance metrics (pearson_r, r2, p_value)
        model_name: Name of the model for plot titles
        plots_dir: Directory to save plots
        config: Optional plot configuration
        null_r: Optional null distribution array for null histogram
        empirical_r: Optional empirical r value for null histogram
        bootstrap_results: Optional bootstrap results dictionary
        cal_metrics: Optional calibration metrics dictionary
...
def visualize_time_generalization(
    ...
    """
    Create visualizations for time-generalization analysis.
    
    Generates time-generalization matrix plots with optional null distribution overlay.
    
    Args:
        tg_matrix: Time-generalization matrix (n_train_windows, n_test_windows)
        window_centers: Array of window center times in seconds
        plots_dir: Directory to save plots
        metric: Metric name for colorbar label (default: "r")
        null_matrix: Optional null distribution matrix for significance masking
        config: Optional plot configuration
        logger: Optional logger instance
    """
    if logger is None:
...
def visualize_model_comparisons(
    ...
    Generates model comparison plots showing performance metrics across models.
    
    Args:
        models_dict: Dictionary mapping model names to their metrics dictionaries
        plots_dir: Directory to save plots
        config: Optional plot configuration
        logger: Optional logger instance
    """
...
def visualize_riemann_analysis(
    ...
    """
    Create visualizations for Riemann-based decoding analysis.
    
    Generates band comparison and sliding window plots.
    
    Args:
        band_results: Optional dictionary mapping band names to performance metrics
        sliding_df: Optional DataFrame with sliding window results
        plots_dir: Directory to save plots
        config: Optional plot configuration
        logger: Optional logger instance
    """
...
def visualize_incremental_validity(
    ...
    """
    Create visualizations for incremental validity analysis.
    
    Generates plots comparing models with and without temperature predictor.
    
    Args:
        inc_summary: Dictionary with incremental validity summary statistics
        plots_dir: Directory to save plots
        config: Optional plot configuration
        logger: Optional logger instance
    """
