...
Because LOSO folds vary in the number of retained temperature levels and thermode surfaces, the Level 2 design matrix may approach rank deficiency for some training folds. If the training-fold design matrix condition number exceeded $10^{10}$, a prespecified fallback hierarchy was applied: Model A (full design), Model B (drop peripheral low-gamma power and residual ECG coupling), Model C (further drop HRF-weighted DVARS), Model D (retain only intercept, temperature, block, and trial onset time). The highest-rank model whose condition number fell below threshold was used. The fallback model index was logged for every fold.
...
Spectral power features were extracted using Morlet wavelet decomposition with frequency-adaptive cycle counts (cycles $= f / 2.0$, bounded between 3.0 and 15.0) and decimation factor 4. To isolate induced (non-phase-locked) oscillatory activity, the evoked response was subtracted from each trial prior to decomposition. The primary ERP estimate was the fold-level grand average across all training trials, pooling conditions. This preserves condition-mean evoked differences in the residual signal, ensuring the predictive model can access condition-related evoked variance. Condition-specific ERP subtraction (i.e., subtracting the mean evoked response within each condition separately) was evaluated as a sensitivity analysis; because condition-specific subtraction removes exactly the evoked variance that differs between pain and non-pain trials, it may attenuate the signal of interest and was therefore not primary. The ERP was computed exclusively from training-fold trials to prevent leakage.
...
fMRI targets were normalized using the Yeo-Johnson power transformation fitted on training targets only. Predictions were inverse-transformed before reporting primary metrics. Both the numerator and denominator of the out-of-sample $R^2$ were computed on the original (untransformed) target scale after inverse-transforming predictions, ensuring that the training-fold mean baseline ($\bar{y}_{train,f}$) and the prediction residuals are in commensurate units. The ElasticNet objective was:
...
Statistical inference used nonparametric permutation testing (5,000 permutations). The permutation null operated on nuisance-residualized targets: within each fold, residual targets were circularly shifted relative to EEG features within each run (minimum shift distance of 5 trials), approximately preserving within-run temporal autocorrelation (Winkler et al., 2014). The circular-shift null preserves intra-run temporal dependencies but does not capture inter-run systematic trends (e.g., sensitization or habituation across runs); such trends are partially controlled by the Level 2 block regressor. For each permutation, the full prediction pipeline was refit. Within-block random shuffling was evaluated as a sensitivity analysis.
...
Supplementary models were trained to predict each Level 2 nuisance variable and subjective pain ratings from the same EEG features. Permutation p-values for nuisance-prediction models were Holm-corrected across the full set of supplementary targets. Strong prediction of nuisance variables indicated caution that the model may track task structure or acquisition noise. Strong prediction of ratings was interpreted as construct overlap with pain report. The Level 2 framework provides the primary evidence: EEG features must predict stimulus- and acquisition-controlled fMRI pain-signature expression above chance before any pain-relevant interpretation is made.
