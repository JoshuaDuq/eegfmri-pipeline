"""Write the standalone Study 1 SIIPS1 behavioral-validity SVG."""
...
OUTPUT_FILENAME = "siips1_behavioral_validity.svg"


def write_siips1_behavioral_validity(
    *,
    summary: BehavioralValiditySummary,
    config: Any,
) -> Path:
    require_behavioral_validity_target(summary, SIIPS1_SPECIFICATION.target)
    figure = build_behavioral_validity_figure(
        summary,
        color_config_key=SIIPS1_SPECIFICATION.color_config_key,
        config=config,
    )
...
def main(argv: Sequence[str] | None = None) -> Path:
    parser = argparse.ArgumentParser(
        description="Write the Study 1 SIIPS1 behavioral-validity SVG."
    )
    ...
    summary = build_behavioral_validity_summary(
        ...
        specification=SIIPS1_SPECIFICATION,
        config=config,
    )
    output_path = write_siips1_behavioral_validity(summary=summary, config=config)
    print(output_path)
    return output_path
...
__all__ = ["main", "write_siips1_behavioral_validity"]
