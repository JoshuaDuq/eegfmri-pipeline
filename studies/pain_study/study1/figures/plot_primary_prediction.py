"""Write the standalone Study 1 primary-prediction estimation figure."""

from __future__ import annotations

import argparse
from collections.abc import Sequence
from dataclasses import dataclass
from pathlib import Path
from typing import Any

from eeg_pipeline.infra.tsv import write_parquet, write_tsv
from eeg_pipeline.utils.config.loader import load_config, require_config_value
from studies.pain_study.study1.cohort import study1_output_root
from studies.pain_study.study1.config.loader import apply_study1_config_defaults
from studies.pain_study.study1.figures.primary_prediction import (
    load_primary_prediction_summary,
)
from studies.pain_study.study1.figures.primary_prediction_plot import (
    build_primary_prediction_figure,
)
from studies.pain_study.study1.figures.validity_style import (
    save_publication_svg,
    validity_output_dir,
)

OUTPUT_FILENAME = "primary_prediction_estimation.svg"
SUBJECT_AUDIT_FILENAME = "primary_prediction_by_subject.tsv"
SUMMARY_AUDIT_FILENAME = "primary_prediction_summary.tsv"


@dataclass(frozen=True)
class PrimaryPredictionFigurePaths:
    """Figure and audit paths written by the primary-prediction stage."""

    svg: Path
    subject_tsv: Path
    subject_parquet: Path
    summary_tsv: Path
    summary_parquet: Path


def write_primary_prediction(
    *,
    report_path: Path,
    config: Any,
    output_path: Path | None = None,
) -> PrimaryPredictionFigurePaths:
    """Validate inputs and write the SVG plus four audit files."""

    summary = load_primary_prediction_summary(report_path, config)
    resolved_output = output_path or validity_output_dir(config) / OUTPUT_FILENAME
    figure = build_primary_prediction_figure(summary, config)
    save_publication_svg(
        figure,
        resolved_output,
        config,
        dimensions_mm=require_config_value(
            config,
            "study1.figures.primary_prediction.dimensions_mm",
        ),
    )

    subject_tsv = resolved_output.with_name(SUBJECT_AUDIT_FILENAME)
    subject_parquet = subject_tsv.with_suffix(".parquet")
    summary_tsv = resolved_output.with_name(SUMMARY_AUDIT_FILENAME)
    summary_parquet = summary_tsv.with_suffix(".parquet")
    write_tsv(summary.participant_performance, subject_tsv)
    write_parquet(summary.participant_performance, subject_parquet)
    write_tsv(summary.cohort_performance, summary_tsv)
    write_parquet(summary.cohort_performance, summary_parquet)
    return PrimaryPredictionFigurePaths(
        svg=resolved_output,
        subject_tsv=subject_tsv,
        subject_parquet=subject_parquet,
        summary_tsv=summary_tsv,
        summary_parquet=summary_parquet,
    )


def main(argv: Sequence[str] | None = None) -> PrimaryPredictionFigurePaths:
    parser = argparse.ArgumentParser(
        description="Write the Study 1 primary-prediction estimation SVG and audits."
    )
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--study1-config", type=Path)
    parser.add_argument("--report", type=Path)
    parser.add_argument("--output", type=Path)
    arguments = parser.parse_args(argv)

    config = load_config(arguments.config)
    apply_study1_config_defaults(config, arguments.study1_config)
    report_path = arguments.report or study1_output_root(config) / "reports" / "study1_report.tsv"
    output_paths = write_primary_prediction(
        report_path=report_path,
        config=config,
        output_path=arguments.output,
    )
    print(output_paths.svg)
    return output_paths


if __name__ == "__main__":
    main()


__all__ = [
    "PrimaryPredictionFigurePaths",
    "main",
    "write_primary_prediction",
]
