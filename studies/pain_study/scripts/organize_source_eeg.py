...
MAIN_COHORT_SUBJECTS = ("0000", "0001") + tuple(f"{subject:04d}" for subject in range(3, 16))
...
def organize_subject_eeg(
    ...
    if original_destination.exists() or processed_destination.exists():
        raise FileExistsError(f"EEG source is already organized: {eeg_directory}")

    original_sources = _triplets(participant_raw, 5_000.0)
    processed_sources = _triplets(eeg_directory, 1_000.0, recursive=True)
...
def organize_cohort(kingston_root: Path, source_data_root: Path) -> tuple[OrganizationResult, ...]:
    """Organize the fixed main simultaneous EEG-fMRI cohort."""
    return tuple(
        ...
        for subject in MAIN_COHORT_SUBJECTS
    )
...
def main() -> None:
    ...
    for result in organize_cohort(arguments.kingston_root, arguments.source_data_root):
        print(
