...
Reads only what ``line-comb diagnose --stage analyse`` wrote, so the plots can be
...
def run(args: argparse.Namespace) -> None:
    """Draw the figures. Split from ``main`` so the CLI command can call it with its own args."""
    ...
    args.input_dir = config.path("diagnosis_dir", override=args.input_dir)
