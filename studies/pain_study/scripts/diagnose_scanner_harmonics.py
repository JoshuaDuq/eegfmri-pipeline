...
DEFAULT_DERIV_ROOT = Path("/Volumes/KINGSTON/EEG_fMRI_data/derivatives/preprocessed/eeg")
DEFAULT_SOURCE_ROOT = Path("/Volumes/KINGSTON/EEG_fMRI_data/source_data")
DEFAULT_BIDS_ROOT = Path("/Volumes/KINGSTON/EEG_fMRI_data/bids_output/eeg")
DEFAULT_OUTPUT_DIR = Path("outputs/scanner_harmonic_diagnosis")
...
def comb_structure(lines: pd.DataFrame, tolerance_hz: float = 0.06) -> pd.DataFrame:
    ...
    if members.sum() >= 3:
        fit = hd.fit_arithmetic_comb(narrow[members])
        ...
        rows.append(
            {
                ...
                "free_intercept_hz": fit.intercept_hz,
