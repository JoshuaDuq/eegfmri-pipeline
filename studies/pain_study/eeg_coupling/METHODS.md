# EEG Coupling - Manuscript-Style Methods for Trial-Wise Cortical EEG-BOLD Coupling

This document is the publication-style Methods companion for EEG coupling. It is
implementation-grounded: every methodological statement below reflects the current
behavior of the EEG-BOLD coupling workflow in `studies/pain_study/eeg_coupling/`.
...
EEG coupling asks whether trial-wise fluctuations in source-localized cortical EEG power
...
The primary EEG coupling objective is to test whether trial-wise cortical EEG power covaries
...
If EEG coupling is started from raw data, the upstream sources are:
...
EEG coupling itself does not perform those conversions.
...
For the production EEG coupling nuisance model, the same table must also contain the
...
EEG coupling depends on the pain-paradigm event schema because plateau selection,
...
The production EEG coupling analysis selects plateau-phase stimulation rows through
...
EEG coupling disables dynamic ROI building at runtime and instead uses a frozen ROI library
declared in:

```text
studies/pain_study/eeg_coupling/config/roi_library/eeg_coupling/fsaverage/eeg_coupling_roi_manifest.json
```
...
| --- | --- | --- | --- |
| `right_operculo_periinsular` | right | union of right long insular/subcentral and circular insular labels in `aparc.a2009s` | `rh.eeg_coupling_right_operculo_parietal.label` |
| `midcingulate_pre_sma` | bilateral | bilateral mid-anterior cingulate plus a custom medial pre-SMA extension constrained from `BA6_exvivo.thresh.label` | `lh.eeg_coupling_cingulo_pre_sma.label`, `rh.eeg_coupling_cingulo_pre_sma.label` |
...
EEG coupling invokes the shared trial-signature extraction machinery in least-squares
...
`events_file` and `events_row`, because EEG coupling reconstructs trial history from the
...
When trial-wise effect variances are available, EEG coupling also computes the variance of
...
Because the LSS `trials.tsv` retains `events_file` and `events_row`, EEG coupling rebuilds
...
with the `nlme::lme` backend through the EEG coupling R wrapper. The production model uses:
...
The production EEG coupling configuration is not the CLI default. A production run should
therefore pass the EEG coupling YAML explicitly.
...
```text
studies/pain_study/eeg_coupling/config/eeg_bold_coupling_smoketest.yaml
```

This smoke file is useful for pipeline validation but it is not the frozen scientific
specification used for EEG coupling.

The production YAML intentionally sets `paths.signature_maps: []`. Consequently, the
main EEG coupling analysis does not require signature maps, local signature-expression
...
EEG coupling deliberately surfaces these conditions as explicit errors rather than attempting
...
EEG coupling. A manuscript writer still needs a second class of information: facts that are
