...
Deleting `volume_marker_description` orphans `run_evidence.py`'s `volume_description=settings.volume_marker_description` argument, so that argument goes. **The `volume_description` parameter of `compute_run_continuity` STAYS**, with the marker literal inlined in `continuity.py` as a temporary default. Removing it here would leave `volume_onsets` permanently empty, hence `has_volume_markers` permanently False, hence `acquisition_context_of` classifying every participant OUT_OF_SCANNER — silently, with no error — for every task between this one and Task 17. That is the exact failure Task 4 was amended to avoid. The parameter and the inlined literal leave in Task 17, together with `has_volume_markers` and `AcquisitionContext`.
