...
def find_clean_epoch_files(pipeline_path, task):
    root = Path(pipeline_path)
    patterns = (
        f"sub-*/eeg/sub-*_task-{task}_proc-clean_epo.fif",
        f"sub-*/ses-*/eeg/sub-*_ses-*_task-{task}_proc-clean_epo.fif",
    )
    clean_epoch_files = {
        path
        for pattern in patterns
        for path in root.glob(pattern)
        if path.is_file() and path.parent.name == "eeg"
    }
    return sorted(str(path) for path in clean_epoch_files)
...
def collect_preprocessing_stats(bids_path, pipeline_path, task):
    ...
    clean_epo_files = find_clean_epoch_files(pipeline_path, task)
    ...
    for p in clean_epo_files:
        ...
        logger.info(
            **gen_log_kwargs(message=msg, subject=sub_num.replace("sub", ""), session=sess_num)
        )

        preprocessing_stats.loc[(sub_num, sess_num), "n_bad_channels"] = len(chan_file)

        components_path = utils.get_derived_path(
            p, "_proc-clean_epo.fif", "_proc-ica_components.tsv"
        )
        ...
        n_boundary = len([i for i, x in enumerate(epochs.drop_log) if "BAD boundary" in x])
        ...
        for event in events_type:
            ...
            preprocessing_stats.loc[(sub_num, sess_num), event + "_total_clean_epochs"] = len(
                epochs_cond
            )
