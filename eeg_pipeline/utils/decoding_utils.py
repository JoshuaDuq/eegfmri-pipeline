...
def _load_decoding_constants(config=None):
    ...
    if cv_config is None:
        raise ValueError("decoding.cv not found in config.")
    
    constants_config = config.get("decoding.constants")
    if constants_config is None:
        raise ValueError("decoding.constants not found in config.")
    
    preprocessing_config = config.get("decoding.preprocessing")
    if preprocessing_config is None:
        raise ValueError("decoding.preprocessing not found in config.")
    
    models_config = config.get("decoding.models")
    if models_config is None:
        raise ValueError("decoding.models not found in config.")
    
    analysis_config = config.get("decoding.analysis")
    if analysis_config is None:
        raise ValueError("decoding.analysis not found in config.")
    
    elasticnet_config = models_config.get("elasticnet")
    if elasticnet_config is None:
...
def create_elasticnet_pipeline(seed: int = 42, config: Optional[Dict[str, Any]] = None) -> Pipeline:
    constants = _load_decoding_constants(config)
...
def create_rf_pipeline(n_estimators: Optional[int] = None, n_jobs: int = 1, seed: int = 42, config: Optional[Dict[str, Any]] = None) -> Pipeline:
    constants = _load_decoding_constants(config)
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        for warning in w:
            if isinstance(warning.message, ConvergenceWarning):
                msg = f"Fold {fold_info}: ConvergenceWarning - {warning.message}"
                if logger:
                    logger.warning(msg)
                else:
                    print(f"[WARNING] {msg}")
            elif isinstance(warning.message, ConstantInputWarning):
                msg = f"Fold {fold_info}: ConstantInputWarning - {warning.message}"
                if logger:
                    logger.warning(msg)
                else:
                    print(f"[WARNING] {msg}")
...
def grid_search_with_warning_logging(grid, X, y, fold_info: str = "", logger=None, **fit_params):
    with warnings.catch_warnings(record=True) as w:
        ...
        for warning in w:
            if isinstance(warning.message, ConvergenceWarning):
                msg = f"Fold {fold_info}: ConvergenceWarning during GridSearchCV - {warning.message}"
                if logger:
                    logger.warning(msg)
                else:
                    print(f"[WARNING] {msg}")
            elif isinstance(warning.message, ConstantInputWarning):
                msg = f"Fold {fold_info}: ConstantInputWarning during GridSearchCV - {warning.message}"
                if logger:
                    logger.warning(msg)
                else:
                    print(f"[WARNING] {msg}")
...
def create_stratified_cv_by_binned_targets(y: np.ndarray, n_splits: Optional[int] = None, n_bins: Optional[int] = None, random_state: int = 42, config: Optional[Dict[str, Any]] = None):
    constants = _load_decoding_constants(config)
    if n_splits is None:
...
def create_block_aware_cv_for_within_subject(blocks: np.ndarray, n_splits: Optional[int] = None, random_state: int = 42, config: Optional[Dict[str, Any]] = None):
    constants = _load_decoding_constants(config)
    if n_splits is None:
        n_splits = constants["default_n_splits"]
...
def cluster_bootstrap_subjects(df: pd.DataFrame, subject_col: str, n_boot: int, seed: int, func) -> Tuple[float, Tuple[float, float]]:
    rng = np.random.default_rng(seed)
    theta0 = float(func(df))
    subs = df[subject_col].unique().tolist()
    counts = df[subject_col].value_counts().reindex(subs).astype(float)
    if counts.sum() > 0:
        weights = (counts / counts.sum()).to_numpy()
    else:
        weights = np.full(len(subs), 1.0 / max(1, len(subs)))
    
    thetas = np.zeros(n_boot, dtype=float)
    for b in range(n_boot):
        pick = rng.choice(subs, size=len(subs), replace=True, p=weights)
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, groups: np.ndarray) -> Tuple[dict, pd.DataFrame]:
    ...
    r_values = [r for r in r_list if np.isfinite(r) and -0.999 < r < 0.999]
    avg_r_fisher_z = np.nan if len(r_values) == 0 else float(np.tanh(np.mean(np.arctanh(r_values))))
    pooled["avg_subject_r_fisher_z"] = float(avg_r_fisher_z)
    return pooled, per_subject

def bootstrap_pooled_metrics_by_subject(pred_df: pd.DataFrame, n_boot: Optional[int] = None, seed: int = 42, config_dict: Optional[dict] = None, config: Optional[Dict[str, Any]] = None) -> dict:
    rng = np.random.default_rng(seed)
    if n_boot is None:
