...
def _build_design_matrix(
    ...
    if method == "spearman":
        ...
        covariate_data = (
            df[covariate_columns].to_numpy() if covariate_columns else np.empty((n_samples, 0))
        )
    ...
    try:
        ...
        if is_ill_conditioned:
            if logger:
                logger.warning(f"Design matrix ill-conditioned (cond={condition_number:.1e})")
            return None
    except (ValueError, np.linalg.LinAlgError):
        pass
...
def compute_partial_correlations_with_cov_predictor(
    ...
    if has_covariates and has_predictor:
        ...
        if overlap:
            covariates_with_predictor = covariates_with_predictor.drop(
                columns=overlap, errors="ignore"
            )
        covariates_with_predictor_control = pd.concat(
            [covariates_with_predictor, predictor_cov], axis=1
        )
        try:
