...
def save_paired_comparisons(
    summary: PairedComparisonSummary,
    output_dir: Path,
    suffix: str = "",
) -> Path:
    """Save paired comparison results to TSV file.
    
    Args:
        summary: PairedComparisonSummary to save
    ...
    if not df.empty and "q_values" in summary.metadata:
        df["q_value"] = summary.metadata["q_values"]
        df["significant_fdr"] = df["q_value"] < summary.fdr_alpha
    
    fname = f"paired_comparisons{suffix}.tsv"
    out_path = output_dir / fname
    write_tsv(df, out_path)
    
    meta_path = output_dir / f"paired_comparisons{suffix}.metadata.json"
    meta = {
        "n_tests": summary.n_tests,
        "n_significant_raw": summary.n_significant_raw,
        "n_significant_fdr": summary.n_significant_fdr,
        "fdr_alpha": summary.fdr_alpha,
        **{k: v for k, v in summary.metadata.items() if k != "q_values"},
    }
...
def load_paired_comparisons(
    input_dir: Path,
    suffix: str = "",
) -> Optional[pd.DataFrame]:
    """Load paired comparison results from TSV file.
    ...
    Returns:
        DataFrame with paired comparison results or None if not found
    """
