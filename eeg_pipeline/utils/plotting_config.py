...
class PlotConfig:
    ...
    def get_defaults(cls) -> "PlotConfig":
        ...
        """
        try:
            from eeg_pipeline.utils.config_loader import load_config
            config = load_config()
    ...
    def get_figure_size(self, size_name: str, plot_type: Optional[str] = None) -> Tuple[float, float]:
        """Get figure size, with plot-type override if available.
        
        Args:
            size_name: Name of size (e.g., "standard", "square")
            plot_type: Optional plot type for override (e.g., "decoding", "erp")
        
        Returns:
            Tuple of (width, height) in inches
        """
        if plot_type and plot_type in self.plot_type_configs:
            type_config = self.plot_type_configs[plot_type]
            if "figure_size" in type_config:
                size_name = type_config["figure_size"]
        
        size = self.figure_sizes.get(size_name)
        if size:
            return size
        
        return self.figure_sizes.get("standard", (10.0, 8.0))
    
    def get_scatter_marker_size(self, plot_type: Optional[str] = None) -> int:
        """Get scatter marker size based on plot type.
        
        Args:
            plot_type: Plot type (e.g., "decoding", "behavioral")
        ...
        if plot_type and plot_type in self.plot_type_configs:
            type_config = self.plot_type_configs[plot_type]
            marker_size = type_config.get("scatter_marker_size", "default")
            if marker_size == "small":
                return self.style.scatter.marker_size_small
            elif marker_size == "large":
    ...
    def get_line_width(self, width_name: str, plot_type: Optional[str] = None) -> float:
        ...
        Args:
            width_name: Name of width ("thin", "standard", "thick", "bold")
            plot_type: Optional plot type for override
        ...
        if plot_type and plot_type in self.plot_type_configs:
            type_config = self.plot_type_configs[plot_type]
            if "line_width" in type_config:
                width_name = type_config["line_width"]
    ...
    def get_color(self, color_name: str, plot_type: Optional[str] = None) -> str:
        """Get color by name, with plot-type override.
        
        Args:
            color_name: Color name (e.g., "red", "pain", "significant")
            plot_type: Optional plot type for override
        
        Returns:
            Color value (hex string or matplotlib color name)
        """
        if plot_type and plot_type in self.plot_type_configs:
            type_config = self.plot_type_configs[plot_type]
