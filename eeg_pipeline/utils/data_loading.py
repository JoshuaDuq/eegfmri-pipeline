...
def trim_behavioral_to_events_strict(
    ...
    logger.critical(
        ...
        f"Behavioral: {behavioral_len}, Events: {events_len}. "
...
def _match_by_trial_identifiers(
    ...
    behavioral_trial_col = next((c for c in trial_cols_behavioral if c in behavioral_df.columns), None)
    events_trial_col = next((c for c in trial_cols_events if c in events_df.columns), None)
...
def _match_by_temperature(
    ...
    behavioral_temp_col = next((c for c in temp_cols_behavioral if c in behavioral_df.columns), None)
    events_temp_col = next((c for c in temp_cols_events if c in events_df.columns), None)
    events_onset_col = next((c for c in onset_cols_events if c in events_df.columns), None)
...
def _pick_first_column(df: Optional[pd.DataFrame], candidates: List[str]) -> Optional[str]:
    if df is None:
        return None
    from .io_utils import _find_column_unified
    return _find_column_unified("dataframe", "generic", df, column_names=candidates)
...
def extract_default_covariates(events_df: pd.DataFrame, config) -> List[str]:
    ...
    for candidate in trial_column_candidates:
    ...
    if partial_covars:
        for covariate in partial_covars:
            if covariate in df_events.columns:
                ...
                column_name_map[covariate] = canonical_name
                continue
            ...
            if canonical_name == "temperature":
                ...
                if temp_column:
                    covariate_columns.append(temp_column)
                    column_name_map[temp_column] = canonical_name
    else:
        temp_column = _pick_first_column(df_events, temperature_candidates)
        if temp_column:
            covariate_columns.append(temp_column)
            column_name_map[temp_column] = "temperature"
...
def apply_baseline(
    ...
    from .tfr_utils import validate_baseline_window_for_times
    times = np.asarray(epochs.times)
    validate_baseline_window_for_times(times, baseline_window, logger=logger, config=config)
...
def extract_rating_array_for_tf(
    ...
    from .io_utils import _pick_first_column
...
def build_summary_map_for_prefix(
    ...
    from .tfr_utils import build_atlas_rois_from_nodes, build_summary_map_from_roi_nodes
...
def load_channel_correlations(subject: str, band: str, deriv_root: Path, 
    ...
    from .io_utils import deriv_stats_path
...
def extract_channel_importance_from_coefficients(coef_matrix: np.ndarray, feature_names: List[str]) -> pd.DataFrame:
    from eeg_pipeline.utils.decoding_io import parse_pow_feature
...
def prepare_partial_correlation_data(
    ...
    from eeg_pipeline.utils.stats_utils import apply_pooling_strategy
...
def prepare_group_band_roi_data(
    ...
    from eeg_pipeline.utils.stats_utils import prepare_group_data
...
def create_temperature_masks(temp_series: pd.Series, temperature_rounding_decimals: int = 1, min_temperatures_required: int = 2) -> Tuple[Optional[float], Optional[float], Optional[np.ndarray], Optional[np.ndarray]]:
...
def get_temperature_range(temp_series: pd.Series, temperature_rounding_decimals: int = 1, min_temperatures_required: int = 2) -> Tuple[Optional[float], Optional[float]]:
    if temp_series is None:
        return None, None
    s_round = pd.to_numeric(temp_series, errors="coerce").round(temperature_rounding_decimals)
    temps = sorted(map(float, s_round.dropna().unique()))
...
def create_temperature_masks_from_range(temp_series: pd.Series, t_min: float, t_max: float, temperature_rounding_decimals: int = 1) -> Tuple[np.ndarray, np.ndarray]:
...
def validate_sufficient_subjects(groups: np.ndarray, min_subjects: int = 2) -> None:
    ...
    min_subjects : int
        Minimum number of subjects required (default: 2)
