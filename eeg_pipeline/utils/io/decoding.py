...
def export_best_params_long_table(jsonl_path: Path, save_path: Path) -> None:
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            param_configs = [
                ("best_params", "refit_r", None),
                ("best_params_by_r", "grid_best_r", "best_score_r"),
                ("best_params_by_neg_mse", "grid_best_neg_mse", "best_score_neg_mse"),
            ]
            
            for param_key, criterion, score_key in param_configs:
                params = rec.get(param_key)
                if isinstance(params, dict):
                    score = float(rec.get(score_key)) if score_key and rec.get(score_key) is not None else None
                    _add_params_to_records(records, params, model, fold_int, subject_str, criterion, score)
