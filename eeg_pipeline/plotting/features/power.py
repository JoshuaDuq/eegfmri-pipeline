...
def plot_feature_stability_heatmap(
    ...
    """Feature stability heatmap showing CV and lag-1 autocorrelation.
    
    Answers: "Which features are stable across trials?"
    """
    if features_df is None or features_df.empty:
        return
    
    from .utils import compute_feature_stability
    ...
    stability_df = compute_feature_stability(features_df, numeric_cols)
    
    if stability_df.empty:
        return
    
    stability_df = stability_df.sort_values("cv", ascending=True).head(max_features)
    
    fig, axes = plt.subplots(1, 3, figsize=(16, max(6, len(stability_df) * 0.25)))
    
    ax1 = axes[0]
    colors = ["#22C55E" if cv < 0.5 else "#F59E0B" if cv < 1.0 else "#EF4444" 
              for cv in stability_df["cv"]]
    y_pos = range(len(stability_df))
    ax1.barh(y_pos, stability_df["cv"], color=colors, alpha=0.8, edgecolor="white")
    ax1.set_yticks(y_pos)
    ax1.set_yticklabels([f[:25] for f in stability_df["feature"]], fontsize=7)
    ax1.set_xlabel("Coefficient of Variation")
    ax1.set_title("Variability (CV)", fontweight="bold")
    ax1.axvline(0.5, color="gray", linestyle="--", alpha=0.5, label="CV=0.5")
    ax1.axvline(1.0, color="gray", linestyle=":", alpha=0.5, label="CV=1.0")
    ax1.legend(fontsize=8)
    ax1.invert_yaxis()
    
    ax2 = axes[1]
    acf_colors = ["#3B82F6" if abs(acf) > 0.3 else "#94A3B8" for acf in stability_df["lag1_acf"]]
    ax2.barh(y_pos, stability_df["lag1_acf"], color=acf_colors, alpha=0.8, edgecolor="white")
    ax2.set_yticks(y_pos)
    ax2.set_yticklabels([])
    ax2.set_xlabel("Lag-1 Autocorrelation")
    ax2.set_title("Temporal Persistence", fontweight="bold")
    ...
    ax3 = axes[2]
    scatter_colors = stability_df["lag1_acf"].values
    sc = ax3.scatter(stability_df["cv"], stability_df["lag1_acf"], 
                    c=scatter_colors, cmap="RdYlBu_r", alpha=0.7, s=50, edgecolor="white")
    ax3.set_xlabel("CV (Variability)")
    ax3.set_ylabel("Lag-1 ACF (Persistence)")
    ax3.set_title("Stability Profile", fontweight="bold")
    ax3.axhline(0, color="gray", linestyle="-", linewidth=0.5)
    ax3.axvline(0.5, color="gray", linestyle="--", alpha=0.5)
    plt.colorbar(sc, ax=ax3, label="Lag-1 ACF")
    
    ax3.axhspan(0.3, 1.0, xmin=0, xmax=0.5/ax3.get_xlim()[1] if ax3.get_xlim()[1] > 0 else 0.5, 
                alpha=0.1, color="green", label="Stable & Persistent")
    
    fig.suptitle(f"Feature Stability Analysis (sub-{subject})", 
                fontsize=plot_cfg.font.figure_title, fontweight="bold", y=0.98)
    
    footer = f"n={len(features_df)} trials | {len(stability_df)} features shown | Low CV + High ACF = Stable"
    fig.text(0.5, 0.01, footer, ha="center", va="bottom", fontsize=plot_cfg.font.small, color="gray")
    
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    save_fig(fig, save_dir / f"sub-{subject}_feature_stability_heatmap",
             formats=plot_cfg.formats, dpi=plot_cfg.dpi,
             bbox_inches=plot_cfg.bbox_inches, pad_inches=plot_cfg.pad_inches)
    plt.close(fig)
    
    if logger:
        logger.info("Saved feature stability heatmap")
...
def plot_temporal_autocorrelation(
    ...
    """Temporal autocorrelation plot showing trial-to-trial persistence.
    
    Answers: "How correlated are consecutive trials?"
    """
    if features_df is None or features_df.empty:
        return
    
    from .utils import compute_lag1_autocorrelation
    
    plot_cfg = get_plot_config(config)
    
    feature_types = {
        "power": [c for c in features_df.columns if c.startswith("power_")],
        "connectivity": [c for c in features_df.columns if "wpli_" in c or "aec_" in c],
        "microstate": [c for c in features_df.columns if "microstate_" in c or "ms_" in c],
        "aperiodic": [c for c in features_df.columns if "aperiodic_" in c or "aper_" in c],
        "complexity": [c for c in features_df.columns if "_lzc" in c or "hjorth_" in c or "_entropy" in c],
    }
    ...
    for ftype, cols in feature_types.items():
        ...
        for col in cols[:50]:
            ...
            if len(vals) >= 5:
                acf = compute_lag1_autocorrelation(vals)
                if np.isfinite(acf):
                    acfs.append(acf)
        if acfs:
            acf_by_type[ftype] = acfs
    ...
    ax1 = axes[0]
    type_names = list(acf_by_type.keys())
    type_colors = ["#3B82F6", "#22C55E", "#F59E0B", "#8B5CF6", "#EF4444"]
    
    positions = []
    for i, (ftype, acfs) in enumerate(acf_by_type.items()):
        parts = ax1.violinplot([acfs], positions=[i], showmedians=True, widths=0.7)
        color = type_colors[i % len(type_colors)]
        for pc in parts.get("bodies", []):
            pc.set_facecolor(color)
            pc.set_alpha(0.6)
        
        jitter = np.random.uniform(-0.15, 0.15, len(acfs))
        ax1.scatter(i + jitter, acfs, c=color, alpha=0.4, s=15)
        positions.append(i)
    
    ax1.axhline(0, color="black", linestyle="-", linewidth=1)
    ax1.axhline(0.3, color="gray", linestyle="--", alpha=0.5, label="Moderate (±0.3)")
    ax1.axhline(-0.3, color="gray", linestyle="--", alpha=0.5)
    ax1.set_xticks(positions)
    ax1.set_xticklabels([t.capitalize() for t in type_names])
    ax1.set_ylabel("Lag-1 Autocorrelation")
    ax1.set_title("Temporal Persistence by Feature Type", fontweight="bold")
    ax1.legend(fontsize=8)
    ax1.set_ylim(-1, 1)
    ...
    all_acfs = []
    all_labels = []
    for ftype, acfs in acf_by_type.items():
        all_acfs.extend(acfs)
        all_labels.extend([ftype] * len(acfs))
    
    ax2.hist(all_acfs, bins=30, color="#3B82F6", alpha=0.7, edgecolor="white", density=True)
    ax2.axvline(np.mean(all_acfs), color="red", linestyle="--", linewidth=2, 
                label=f"Mean: {np.mean(all_acfs):.3f}")
    ax2.axvline(0, color="black", linestyle="-", linewidth=1)
    ax2.set_xlabel("Lag-1 Autocorrelation")
    ax2.set_ylabel("Density")
    ax2.set_title("Distribution of Temporal Persistence", fontweight="bold")
    ax2.legend()
    
    n_positive = sum(1 for a in all_acfs if a > 0)
    n_negative = sum(1 for a in all_acfs if a < 0)
    ax2.text(0.95, 0.95, f"Positive: {n_positive}\nNegative: {n_negative}", 
            transform=ax2.transAxes, ha="right", va="top", fontsize=10,
            bbox=dict(boxstyle="round", facecolor="white", alpha=0.8))
    
    fig.suptitle(f"Temporal Autocorrelation Analysis (sub-{subject})", 
                fontsize=plot_cfg.font.figure_title, fontweight="bold", y=0.98)
    
    footer = f"n={len(features_df)} trials | {len(all_acfs)} features | Positive ACF = Trial persistence"
    fig.text(0.5, 0.01, footer, ha="center", va="bottom", fontsize=plot_cfg.font.small, color="gray")
    
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    save_fig(fig, save_dir / f"sub-{subject}_temporal_autocorrelation",
             formats=plot_cfg.formats, dpi=plot_cfg.dpi,
             bbox_inches=plot_cfg.bbox_inches, pad_inches=plot_cfg.pad_inches)
    plt.close(fig)
    
    if logger:
        logger.info("Saved temporal autocorrelation plot")


def plot_feature_redundancy_matrix(
    features_df: pd.DataFrame,
    subject: str,
    save_dir: Path,
    logger: logging.Logger,
    ...
    """Feature redundancy matrix showing highly correlated features.
    
    Answers: "Which features are highly correlated and potentially redundant?"
    """
    if features_df is None or features_df.empty:
        return
    ...
    if len(numeric_cols) > max_features:
        ...
        numeric_cols = variances.nlargest(max_features).index.tolist()
    ...
    for i in range(n_features):
        for j in range(i, n_features):
            if i == j:
                ...
                if len(common_idx) >= 5:
                    r, p = spearmanr(vals_i[common_idx], vals_j[common_idx])
                    corr_matrix[i, j] = r
                    corr_matrix[j, i] = r
                    pval_matrix[i, j] = p
                    pval_matrix[j, i] = p
    ...
    if high_corr_pairs:
        ...
        ax2.barh(y_pos, pair_values, color=colors, alpha=0.8, edgecolor="white")
        ax2.set_yticks(y_pos)
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    save_fig(fig, save_dir / f"sub-{subject}_feature_redundancy_matrix",
             formats=plot_cfg.formats, dpi=plot_cfg.dpi,
             bbox_inches=plot_cfg.bbox_inches, pad_inches=plot_cfg.pad_inches)
    plt.close(fig)
    
    if logger:
        logger.info(f"Saved feature redundancy matrix ({len(high_corr_pairs)} redundant pairs)")
