...
def visualize_subject_features(
    ...
    if epochs is not None:
        ...
        if not aper_df.empty:
            plot_aperiodic_topomaps(aper_df, aligned_events, epochs.info, subject, plots_dir, logger, config)
        else:
        ...
        if aligned_events is not None:
            ...
            if not aper_df.empty:
                plot_aperiodic_vs_pain(aper_df, aligned_events, subject, plots_dir, logger, config)
            else:
                logger.warning("Aperiodic features missing; skipping aperiodic slope vs pain plot.")
        plot_aperiodic_r2_histogram(subject, plots_dir, logger, config)
        plot_aperiodic_residual_spectra(subject, plots_dir, logger, config)
        plot_aperiodic_run_trajectories(subject, plots_dir, logger, config)
        ...
        if itpc_df_long is not None and not itpc_df_long.empty:
            ...
            plot_itpc_topomaps(itpc_df_long, epochs.info, subject, plots_dir, logger, config)
        if not itpc_trial_df.empty and aligned_events is not None:
            logger.info("Plotting ITPC-rating scatter grid...")
            plot_itpc_behavior_scatter(itpc_trial_df, aligned_events, subject, plots_dir, logger, config)

    if pac_df is not None and not pac_df.empty:
        logger.info("Plotting PAC comodulograms...")
        plot_pac_comodulograms(pac_df, subject, plots_dir, logger, config)
        if pac_time_df is not None and not pac_time_df.empty:
            logger.info("Plotting time-resolved PAC ribbons...")
            plot_pac_time_ribbons(pac_time_df, subject, plots_dir, logger, config)
        if aligned_events is not None and pac_trials_df is not None and not pac_trials_df.empty:
            ...
            plot_pac_behavior_scatter(pac_trials_df, aligned_events, subject, plots_dir, logger, config)
    ...
    if conn_df is not None and epochs is not None:
        ...
        for lbl in sw_labels:
            ...
            plot_connectivity_heatmap(conn_df, epochs.info, subject, plots_dir, logger, config, prefix, events_df=aligned_events)
            plot_connectivity_network(conn_df, epochs.info, subject, plots_dir, logger, config, prefix, events_df=aligned_events)
    ...
    if epochs is not None:
        ...
        itpc_df_vis, _, itpc_map_vis, itpc_freqs_vis, itpc_times_vis, _ = extract_itpc_features(
            epochs, power_bands, config, logger
        )
        plot_itpc_heatmap(itpc_map_vis, itpc_freqs_vis, itpc_times_vis, subject, plots_dir, logger, config)
        plot_itpc_topomaps(itpc_df_vis, epochs.info, subject, plots_dir, logger, config)
        ...
        if aligned_events is not None and epochs is not None:
            ...
            if template_path.exists():
                try:
                    ...
                    plot_microstate_pain_correlation_heatmap(corr_df, pval_df, subject, plots_dir, logger, config)
