...
def _calculate_y_limits_with_padding(
    values: np.ndarray, padding_factor: float
) -> tuple[float, float]:
    ...
    padded_min = min(0.0, value_min * padding_factor)
    padded_max = value_max * padding_factor
    return padded_min, padded_max


def plot_model_comparison(
    models_dict: dict, save_path: Path, config: Optional[Any] = None
) -> None:
    ...
    create_bar_plot(
        axes[0], x_positions, r_array, model_names, "Pearson's r", plot_cfg
    )
    r_min, r_max = _calculate_y_limits_with_padding(r_array, y_lim_padding)
    axes[0].set_ylim([r_min, r_max])

    create_bar_plot(axes[1], x_positions, r2_array, model_names, "R²", plot_cfg)
    r2_min, r2_max = _calculate_y_limits_with_padding(r2_array, y_lim_padding)
...
def plot_riemann_band_comparison(
    band_results: dict, save_path: Path, config: Optional[Any] = None
) -> None:
    ...
    r_values = np.array(
        [band_results[band].get("pearson_r", np.nan) for band in bands]
    )
    r2_values = np.array([band_results[band].get("r2", np.nan) for band in bands])

    fig, axes = plt.subplots(1, 2, figsize=fig_size)
    x_positions = np.arange(len(bands))

    create_bar_plot(axes[0], x_positions, r_values, bands, "Pearson's r", plot_cfg)
...
def plot_riemann_sliding_window(
    sliding_df: pd.DataFrame, save_path: Path, config: Optional[Any] = None
) -> None:
    ...
    missing_columns = [
        col for col in required_columns if col not in sliding_df.columns
    ]
    if missing_columns:
        logger.warning(
            f"Missing required columns for Riemann sliding window plot: {missing_columns}"
        )
        return
    ...
    _plot_time_series_metric(
        axes[0], time_centers, r_values, "Pearson's r", plot_cfg, marker_size
    )
    _plot_time_series_metric(
        axes[1], time_centers, r2_values, "R²", plot_cfg, marker_size
    )
...
def plot_incremental_validity(
    inc_summary: dict, save_path: Path, config: Optional[Any] = None
) -> None:
    ...
    errors_lower, errors_upper = compute_error_bars_from_ci_dicts(
        metric_values, confidence_intervals
    )
