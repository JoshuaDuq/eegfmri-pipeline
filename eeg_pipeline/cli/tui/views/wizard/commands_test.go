...
func TestBuildPlotItemConfigArgs_PsychometricsEmitsConfiguredColumns(t *testing.T) {
	m := Model{}
	m.plotItems = []PlotItem{{ID: "behavior_psychometrics", Group: "behavior"}}
	m.plotSelected = map[int]bool{0: true}
	m.plotItemConfigs = map[string]PlotItemConfig{
		"behavior_psychometrics": {
			PsychometricsPredictorColumn: "stimulus_temp",
			PsychometricsOutcomeColumn:   "pain_rating",
		},
	}

	args := m.buildPlotItemConfigArgs()
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_psychometrics", "psychometrics_predictor_column", "stimulus_temp"}) {
		t.Fatalf("expected psychometrics predictor args, got: %#v", args)
	}
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_psychometrics", "psychometrics_outcome_column", "pain_rating"}) {
		t.Fatalf("expected psychometrics outcome args, got: %#v", args)
	}
}

func TestBuildPlotItemConfigArgs_TemporalTopomapsEmitComparisonOverrides(t *testing.T) {
	m := Model{}
	m.plotItems = []PlotItem{{ID: "behavior_temporal_topomaps", Group: "behavior"}}
	m.plotSelected = map[int]bool{0: true}
	m.plotItemConfigs = map[string]PlotItemConfig{
		"behavior_temporal_topomaps": {
			ComparisonColumn:                   "condition",
			ComparisonValuesSpec:               "0 1",
			ComparisonLabelsSpec:               "low high",
			BehaviorTemporalStatsFeatureFolder: "power",
		},
	}

	args := m.buildPlotItemConfigArgs()
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_temporal_topomaps", "comparison_column", "condition"}) {
		t.Fatalf("expected temporal topomap comparison_column args, got: %#v", args)
	}
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_temporal_topomaps", "comparison_values", "0", "1"}) {
		t.Fatalf("expected temporal topomap comparison_values args, got: %#v", args)
	}
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_temporal_topomaps", "comparison_labels", "low", "high"}) {
		t.Fatalf("expected temporal topomap comparison_labels args, got: %#v", args)
	}
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_temporal_topomaps", "temporal_stats_feature_folder", "power"}) {
		t.Fatalf("expected temporal topomap stats feature folder args, got: %#v", args)
	}
}

func TestGetBehaviorScatterSegments_UsesSelectedFeatureWindows(t *testing.T) {
	m := Model{
		availableWindows: []string{"baseline", "active"},
		availableWindowsByFeature: map[string][]string{
			"power": {"baseline", "plateau"},
			"itpc":  {"active", "rebound"},
		},
	}

	got := m.GetBehaviorScatterSegments("power itpc")
	want := []string{"baseline", "plateau", "active", "rebound"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("expected feature-derived segments %v, got %v", want, got)
	}
}

func TestBuildPlotItemConfigArgs_BehaviorScatterEmitsControlOverrides(t *testing.T) {
	m := Model{}
	m.plotItems = []PlotItem{{ID: "behavior_scatter", Group: "behavior"}}
	m.plotSelected = map[int]bool{0: true}
	controlPredictor := false
	controlTrial := true
	m.plotItemConfigs = map[string]PlotItemConfig{
		"behavior_scatter": {
			BehaviorScatterControlPredictor:     &controlPredictor,
			BehaviorScatterControlTrialOrder:    &controlTrial,
			BehaviorScatterPredictorControlMode: "linear",
		},
	}

	args := m.buildPlotItemConfigArgs()
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_scatter", "scatter_control_predictor", "false"}) {
		t.Fatalf("expected scatter_control_predictor args, got: %#v", args)
	}
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_scatter", "scatter_control_trial_order", "true"}) {
		t.Fatalf("expected scatter_control_trial_order args, got: %#v", args)
	}
	if !containsSubsequence(args, []string{"--plot-item-config", "behavior_scatter", "scatter_predictor_control_mode", "linear"}) {
		t.Fatalf("expected scatter_predictor_control_mode args, got: %#v", args)
	}
}
