...
_MODEL_CONFIG_OVERRIDES = [
    ...
    (
        ...
        "machine_learning.models.random_forest.min_samples_split_grid",
        lambda v: [int(float(x)) for x in v],
    ),
    (
        "rf_min_samples_leaf_grid",
        "machine_learning.models.random_forest.min_samples_leaf_grid",
        lambda v: [int(float(x)) for x in v],
    ),
    ("rf_bootstrap", "machine_learning.models.random_forest.bootstrap", bool),
    (
        "variance_threshold_grid",
    ...
    (
        ...
        "machine_learning.preprocessing.power_transformer_standardize",
        bool,
    ),
    ...
    (
        ...
        "machine_learning.preprocessing.feature_selection_percentile",
        float,
    ),
    ...
    (
        "lr_c_grid",
        "machine_learning.models.logistic_regression.C_grid",
        lambda v: [float(x) for x in v],
    ),
    ...
    ("eval_subject_weighting", "machine_learning.evaluation.subject_weighting", str),
    ...
    (
        "data_max_excluded_subject_fraction",
        "machine_learning.data.max_excluded_subject_fraction",
        float,
    ),
    ("incremental_baseline_alpha", "machine_learning.incremental_validity.baseline_alpha", float),
    (
        "incremental_require_baseline_predictors",
        "machine_learning.incremental_validity.require_baseline_predictors",
        bool,
    ),
    ("interpretability_grouped_outputs", "machine_learning.interpretability.grouped_outputs", bool),
    (
        "timegen_min_subjects",
        "machine_learning.analysis.time_generalization.min_subjects_per_cell",
        int,
    ),
    (
        "timegen_min_valid_perm_fraction",
        "machine_learning.analysis.time_generalization.min_valid_permutation_fraction",
        float,
    ),
    (
        "class_min_subjects_for_auc",
        "machine_learning.classification.min_subjects_with_auc_for_inference",
        int,
    ),
    (
        "class_max_failed_fold_fraction",
        "machine_learning.classification.max_failed_fold_fraction",
        float,
    ),
    (
        "strict_regression_continuous",
        "machine_learning.targets.strict_regression_target_continuous",
        bool,
    ),
]
...
def _update_model_config(args: argparse.Namespace, config: Any) -> None:
    ...
    if args.rf_max_depth_grid is not None:
        config["machine_learning.models.random_forest.max_depth_grid"] = _parse_max_depth_values(
            args.rf_max_depth_grid
        )
    ...
    if getattr(args, "svm_class_weight", None) is not None:
        config["machine_learning.models.svm.class_weight"] = (
            None if args.svm_class_weight == "none" else args.svm_class_weight
        )
    if getattr(args, "lr_class_weight", None) is not None:
        config["machine_learning.models.logistic_regression.class_weight"] = (
            None if args.lr_class_weight == "none" else args.lr_class_weight
        )
    if getattr(args, "rf_class_weight", None) is not None:
        config["machine_learning.models.random_forest.class_weight"] = (
            None if args.rf_class_weight == "none" else args.rf_class_weight
        )
...
_ML_PLOT_CONFIG_OVERRIDES = [
    ...
    (
        "ml_plot_formats",
        "machine_learning.plotting.formats",
        lambda v: [str(x).strip().lower() for x in v],
    ),
