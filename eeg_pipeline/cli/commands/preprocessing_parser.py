...
def setup_preprocessing(subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "mode",
        choices=["bad-channels", "ica", "epochs"],
        help="Preprocessing mode: bad-channels, ica, or epochs",
    )
    ...
    prep_group.add_argument(
        "--montage", type=str, default=None, help="EEG montage name (e.g., easycap-M1)"
    )
    prep_group.add_argument(
        "--ch-types", type=str, default=None, help="Channel types to include (e.g., 'eeg')"
    )
    prep_group.add_argument(
        "--eeg-reference", type=str, default=None, help="EEG reference type (e.g., 'average')"
    )
    prep_group.add_argument(
        "--eog-channels",
        type=str,
        default=None,
        help="EOG channels (comma-separated, e.g., 'Fp1,Fp2')",
    )
    prep_group.add_argument(
        "--ecg-channels", type=str, default=None, help="ECG channels (comma-separated, e.g., 'ECG')"
    )
    prep_group.add_argument(
        "--random-state", type=int, default=None, help="Random seed for reproducibility"
    )
    ...
    prep_group.add_argument(
        "--line-freq", type=int, help="Line frequency for EEG data (Hz), typically 50 or 60"
    )

    # ICA
    prep_group.add_argument(
        "--spatial-filter", choices=["ica", "ssp"], help="Spatial filter type (ica or ssp)"
    )
    prep_group.add_argument(
        "--ica-method",
    ...
    prep_group.add_argument(
        "--ica-labels-to-keep", nargs="+", help="ICA component labels to keep (e.g., brain other)"
    )
    ...
    prep_group.add_argument(
        "--repeats", type=int, default=3, help="Number of bad channel detection iterations"
    )
    prep_group.add_argument(
    ...
    prep_group.add_argument(
        "--file-extension", type=str, default=".vhdr", help="File extension for EEG data"
    )
    prep_group.add_argument(
        "--rename-anot-dict",
        type=str,
        default=None,
        help='JSON dict for annotation renaming, e.g. \'{"BAD boundary":"BAD_boundary"}\'',
    )
    prep_group.add_argument(
        "--custom-bad-dict",
        type=str,
        default=None,
        help="JSON dict of custom bad channels per task/subject",
    )
    prep_group.add_argument(
    ...
    prep_group.add_argument(
        "--breaks-min-length", type=int, default=20, help="Minimum break duration in seconds"
    )

    # Event timing
    prep_group.add_argument(
        "--t-start-after-previous", type=int, default=2, help="Time after previous event in seconds"
    )
    prep_group.add_argument(
        "--t-stop-before-next", type=int, default=2, help="Time before next event in seconds"
    )
    ...
    prep_group.add_argument(
        "--no-baseline", action="store_true", help="Disable epoch baseline correction"
    )
    prep_group.add_argument(
        "--reject", type=float, help="Peak-to-peak amplitude rejection threshold (µV)"
    )
    prep_group.add_argument(
        "--reject-method",
        choices=["none", "autoreject_local", "autoreject_global"],
        help="Epoch rejection method",
    )
    prep_group.add_argument(
    ...
    prep_group.add_argument(
        ...
        type=str,
        default=None,
        help="events.tsv candidate columns for predictor",
    )
    prep_group.add_argument(
        "--event-col-outcome",
        nargs="+",
        type=str,
        default=None,
        help="events.tsv candidate columns for the behavioral outcome",
    )
    prep_group.add_argument(
        ...
        type=str,
        default=None,
        help="events.tsv candidate columns for binary-outcome split",
    )
    prep_group.add_argument(
        "--event-col-required",
        nargs="+",
