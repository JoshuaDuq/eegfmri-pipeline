...
def _resolve_temporal_target_label(ctx: Any) -> str:
    ...
    if aligned_events is not None:
        resolved = str(resolve_outcome_column(aligned_events, ctx.config) or "").strip()
        if resolved:
            return resolved
...
def stage_temporal_stats_impl(
    ...
    *,
    ...
    if all_temporal_records:
        ...
        target_label = _resolve_temporal_target_label(ctx)
