...
def parse_feature_name(name: str) -> FeatureName:
    """
    ...
    parts = name.split("_")
    if not parts:
        return FeatureName(domain="unknown")
    
    domain = parts[0]
    
    # Simple heuristic parsing - domain determines structure
    if domain in ("power", "erds", "temp", "comp"):
        # power_band_channel_time_stat
        return FeatureName(
            domain=domain,
        ...
        # conn_measure_band_location_time_stat
        return FeatureName(
            domain=domain,
        ...
        # phase_measure_band_channel_time_stat
        return FeatureName(
            domain=domain,
            measure=parts[1] if len(parts) > 1 else None,
            band=parts[2] if len(parts) > 2 else None,
        ...
        # ms_measure_state_time
        return FeatureName(
            domain=domain,
            measure=parts[1] if len(parts) > 1 else None,
            location=parts[2] if len(parts) > 2 else None,  # state as location
            time=parts[3] if len(parts) > 3 else None,
        )
    elif domain == "gfp":
        # gfp_[band]_time_stat
        if len(parts) == 3:
            return FeatureName(domain=domain, time=parts[1], statistic=parts[2])
        return FeatureName(
            domain=domain,
            band=parts[1] if len(parts) > 1 else None,
        ...
        return FeatureName(
            domain=domain,
            measure=parts[1] if len(parts) > 1 else None,  # subdomain
            band=parts[2] if len(parts) > 2 else None,
            location=parts[3] if len(parts) > 3 else None,
            time=parts[4] if len(parts) > 4 else None,
            statistic=parts[5] if len(parts) > 5 else None,
        ...
        return FeatureName(
            domain=domain,
            measure="_".join(parts[1:]) if len(parts) > 1 else None,
        )
