...
def _apply_spatial_transform(
    ...
    try:
        lambda2 = float(
            config.get("feature_engineering.spatial_transform_params.lambda2", DEFAULT_CSD_LAMBDA2)
        )
        stiffness = float(
            config.get(
                "feature_engineering.spatial_transform_params.stiffness", DEFAULT_CSD_STIFFNESS
            )
        )
...
def _compute_psd_with_qc(
    ...
    if baseline_mask is not None and np.any(baseline_mask):
        ...
        window_type = (
            config.get("name") if hasattr(config, "get") and config.get("name") else "baseline"
        )
    else:
...
def precompute_data(
    ...
    *,
    ...
    if compute_psd_data:
        baseline_mask = (
            precomputed.windows.baseline_mask if precomputed.windows is not None else None
        )
...
def _determine_frequency_bands(
    ...
    *,
    ...
    use_iaf = hasattr(config, "get") and bool(
        config.get("feature_engineering.bands.use_iaf", False)
    )
...
def _compute_and_store_bands(
    ...
    band_definitions = [(band, frequency_bands[band]) for band in bands if band in frequency_bands]
