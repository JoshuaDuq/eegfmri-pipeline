...
def test_feature_pipeline_writes_features_to_override_root(tmp_path) -> None:
    ...
    with patch(
        "eeg_pipeline.pipelines.features.validate_rest_configuration"
    ), patch(
    ...
    ) as load_epochs, patch(
        "eeg_pipeline.pipelines.features._load_events_df",
        return_value=None,
    ), patch(
        "eeg_pipeline.pipelines.features.extract_all_features",
        return_value=SimpleNamespace(aper_qc=None, ratios_df=None, asymmetry_df=None, quality_df=None),
    ), patch(
